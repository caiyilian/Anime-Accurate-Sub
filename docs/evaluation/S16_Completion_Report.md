...
- `python -m pytest tests -q --disable-warnings`：72/72 通过。
...
《轻音少女》第一季 14 集、5067 个片段已在无广告源 `data/轻音少女_全集` 上跑通。中文参考
覆盖率为 97.36%，corpus char-F1 为 0.6711、chrF 为 0.3923；日文 ASR corpus CER 为
24.45%。翻译批次使用稳定行 ID，避免模型漏行后译文错绑时间轴；Sakura 校验失败时使用同服
GalTransl 重试，整季共有 13/5067 行进入兜底与人工复核队列。

结果表明当前最大的准确率瓶颈仍是 ASR，而不是模型连通性：远程 Sakura 14B 和同服务器
`crosery/GalTransl-7B-v2.6:Q6_k` 均已实际调用；本机 `11435` 与 `11434` 的
