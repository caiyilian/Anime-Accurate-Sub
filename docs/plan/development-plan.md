...
| S11.3 | **GEMBA-MQM 翻译质量评估** — 双裁判结构化评分 + 总编修订 + 双重复评安全门禁 |
...
**S11.3 验收**：
- [x] Flash Lite / DeepSeek 独立输出四维 MQM 评分、严重度、证据和总分
- [x] 低分或重大错误经总编修订后，由两位裁判独立复评
- [x] 仅在双方认可、最终最低分和最小提升均达标时自动应用；任一失败保留原译
- [x] 主流程、批处理、Web UI、JSONL 断点和完整审计报告
