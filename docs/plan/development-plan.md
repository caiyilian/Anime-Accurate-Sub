...
| S11.2 | **多 Agent 脚本审查** — 5 Agent 并行调用 Ollama + 总编汇总 + 自动修正 |
...
| S13.3 | **外挂字幕优先模式** — 检测内嵌字幕轨道，优先提取已有字幕翻译 | ✅ |
...
| 子阶段 | 内容 | 可并行？|
|--------|------|---------|
| S16.1 | **说话人角色颜色前缀** — ASS 颜色标签 + 角色名映射 | ✅ |
| S16.2 | **Web UI 界面** — Gradio / FastAPI | ✅ |
| S16.3 | **人工校对模式** — 交互式翻译修正 | ✅ |
| S16.4 | **插件系统架构** — 翻译后端/ASR 后端/字幕样式插件接口 | ✅ |
| S16.5 | **视频预览** — 实时字幕效果预览 | ✅ |
