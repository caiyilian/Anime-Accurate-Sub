  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-white/[0.035] p-6">
      <div className="flex items-center justify-between gap-4">
        ...
        {queue.length > 0 && (
          <button type="button" onClick={onClear} className="text-sm text-slate-400 hover:text-rose-300">
            清空
          </button>
        )}
      </div>
      ...
      >
        <span className="block text-base font-medium">拖放 MP4 / MKV，或点击选择多个视频</span>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-500">
                  ...
                  <span>·</span>
                  <button type="button" onClick={() => onAttachSubtitle(video.id)} className="text-cyan-300 hover:text-cyan-200">
              ...
              <div className="flex shrink-0 gap-1">
                <button aria-label={`上移 ${video.name}`} disabled={index === 0} onClick={() => onMove(video.id, -1)} className="rounded-lg px-2 py-1 text-slate-400 hover:bg-white/5 disabled:opacity-20">↑</button>
                <button aria-label={`下移 ${video.name}`} disabled={index === queue.length - 1} onClick={() => onMove(video.id, 1)} className="rounded-lg px-2 py-1 text-slate-400 hover:bg-white/5 disabled:opacity-20">↓</button>
                <button aria-label={`移除 ${video.name}`} onClick={() => onRemove(video.id)} className="rounded-lg px-2 py-1 text-slate-400 hover:bg-rose-400/10 hover:text-rose-300">×</button>
              </div>
            </div>
          </article>
        ))}
