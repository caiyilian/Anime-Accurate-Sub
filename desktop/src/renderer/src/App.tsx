...
export default function App(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    runtime
      ...
      .then(async (value) => {
        ...
        if (active)
          setMessage(result.ready ? '运行环境已就绪。' : '环境仍有阻塞项，请展开设置检查路径。')
      })
      .catch(
        (error) => active && setMessage(error instanceof Error ? error.message : String(error))
      )
    return () => {
      active = false
    }
    ...
    const timer = setTimeout(() => {
      runtime
        ...
        .catch(
          (error) =>
            active &&
            setMessage(`命令预览失败：${error instanceof Error ? error.message : String(error)}`)
        )
    }, 250)
    return () => {
      active = false
  ...
  }, [firstVideo, previewRevision, runtime, settings])
  ...
  const addPaths = useCallback(
    async (paths: string[]): Promise<void> => {
      ...
      try {
        ...
        setMessage(
          `已加入 ${accepted} 个视频${rejected ? `，忽略 ${rejected} 个无效或重复项` : ''}。`
        )
      } catch (error) {
        setMessage(error instanceof Error ? error.message : String(error))
      } finally {
  ...
  const attachSubtitle = async (id: string): Promise<void> => {
    ...
    if (!path) return
    setQueue((current) =>
      current.map((item) => (item.id === id ? { ...item, japaneseSubtitlePath: path } : item))
    )
    setMessage('已关联单集日文字幕。')
  }

  const pickDirectory = async (
    field: 'projectRoot' | 'outputRoot' | 'japaneseSubtitleDir'
  ): Promise<void> => {
    ...
    () =>
      Object.fromEntries(
        pipeline?.jobs.map((job) => [
          job.video.path,
          `${job.status} · ${job.progress.overallPercent.toFixed(0)}%`
        ]) ?? []
      ),
    [pipeline]
  )
    ...
    <div data-testid="desktop-root" className="min-h-screen bg-slate-950 text-slate-100">
        ...
        <div className="mx-auto flex max-w-[1500px] items-center justify-between">
          ...
          <nav aria-label="主导航" className="flex items-center gap-2">
            {navigation.map((item, index) => (
              <span
                key={item}
                className={`rounded-lg px-3 py-2 text-sm ${index === 0 ? 'bg-white/10 text-white' : 'text-slate-500'}`}
              >
                {item}
              </span>
            ))}
      ...
      <main className="mx-auto max-w-[1500px] px-7 py-7">
        <section className="mb-6 flex flex-wrap items-end justify-between gap-5">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-cyan-400/20 bg-cyan-400/8 px-3 py-1 text-xs text-cyan-300">
              D6 / Results preview & export
            </div>
            <h2 className="mt-4 text-3xl font-semibold tracking-tight">字幕生成工作台</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
              先排列视频，再启用项目已经完成的上下文、审查和 MQM
              能力。参考字幕只用于评测；正式生成不依赖字幕组译文。
            </p>
          </div>
          <div className="text-right">
            <p
              className={`font-mono text-sm ${diagnostics?.ready ? 'text-emerald-300' : 'text-amber-300'}`}
            >
              {readySummary}
            </p>
        ...
        <section className="mb-5 flex flex-wrap items-center gap-2 rounded-2xl border border-white/10 bg-slate-900/55 px-4 py-3">
          ...
          <p role="status" className="ml-auto text-sm text-slate-400">
            {message}
          </p>
        </section>
        ...
        <section className="mt-6 rounded-3xl border border-white/10 bg-slate-900/45 p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex gap-2">
              <button
                disabled={busy || !settings || pipelineActive}
                onClick={() => void resetSettings()}
                className="rounded-xl border border-white/10 px-4 py-2 text-sm text-slate-400 hover:bg-white/5 disabled:opacity-40"
              >
                恢复默认
              </button>
              <button
                ...
                className="rounded-xl bg-cyan-400 px-5 py-2 text-sm font-semibold text-slate-950 disabled:opacity-40"
              >
                保存配置
              </button>
              {canResume && (
                <button
                  disabled={busy || pipelineActive}
                  onClick={() => void resumePipeline()}
                  className="rounded-xl border border-amber-400/30 bg-amber-400/10 px-5 py-2 text-sm font-semibold text-amber-200 disabled:opacity-40"
                >
                  继续未完成任务
                </button>
              )}
              {pipelineActive ? (
                <button
                  ...
                  className="rounded-xl bg-rose-400 px-5 py-2 text-sm font-semibold text-slate-950 disabled:opacity-40"
                >
                  取消并保留断点
                </button>
              ) : (
                <button
                  disabled={busy || !settings || !queue.length || !diagnostics?.ready}
                  onClick={() => void startPipeline()}
                  className="rounded-xl bg-emerald-400 px-5 py-2 text-sm font-semibold text-slate-950 disabled:opacity-40"
                >
                  开始完整流程
                </button>
              )}
          ...
          {pipeline && (
            <div className="mt-4 flex flex-wrap gap-2 text-xs text-slate-400">
              ...
              <span className="rounded-full bg-white/5 px-3 py-1">
                成功 {pipeline.jobs.filter((job) => job.status === 'succeeded').length}
              </span>
              <span className="rounded-full bg-white/5 px-3 py-1">
                失败 {pipeline.jobs.filter((job) => job.status === 'failed').length}
              </span>
              <span className="rounded-full bg-white/5 px-3 py-1">
                待处理 {pipeline.jobs.filter((job) => job.status === 'pending').length}
              </span>
            </div>
          )}
          <pre className="mt-4 max-h-40 overflow-auto whitespace-pre-wrap break-all rounded-2xl border border-white/8 bg-slate-950 p-4 font-mono text-xs leading-5 text-slate-400">
            {preview?.display ??
              (queue.length
                ? '正在生成命令预览…'
                : '加入视频后显示首个任务的完整命令；执行时仍使用参数数组，不经过 shell。')}
          </pre>
        </section>
